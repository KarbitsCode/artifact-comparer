...
- downloading artifacts requires a token with `permissions: actions: read`; by default this uses `GITHUB_TOKEN`, but you can pass a different token via the `github-token` input if the default token is too restricted
